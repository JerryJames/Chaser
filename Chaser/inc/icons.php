 ...
 */
function chaser_get_svg( $icon = null ) {
 ...
 */
function chaser_dropdown_icon_to_menu_link( $title, $item, $args, $depth ) {
	if ( 'primary' === $args->theme_location || 'secondary' === $args->theme_location ) {
		foreach ( $item->classes as $value ) {
			if ( 'menu-item-has-children' === $value || 'page_item_has_children' === $value ) {
				$title = $title . chaser_get_svg( 'expand' );
			}
		}
	}
...
add_filter( 'nav_menu_item_title', 'chaser_dropdown_icon_to_menu_link', 10, 4 );
