...
function chaser_theme_addons_setup() {
	...
	add_theme_support( 'infinite-scroll', array(
		...
		'wrapper'        => false,
		'render'         => 'chaser_infinite_scroll_render',
		'posts_per_page' => 6,
	) );
...
add_action( 'after_setup_theme', 'chaser_theme_addons_setup' );


/**
 * Load custom stylesheets for theme addons
 */
function chaser_theme_addons_scripts() {
...
add_action( 'wp_enqueue_scripts', 'chaser_theme_addons_scripts' );


/**
 * Custom render function for Infinite Scroll.
 */
function chaser_infinite_scroll_render() {
	while ( have_posts() ) {
		the_post();
 ...
 */
function chaser_amp_menu_toggle() {
	if ( chaser_is_amp() ) {
 ...
 */
function chaser_amp_menu_is_toggled() {
	if ( chaser_is_amp() ) {
 ...
 */
function chaser_amp_menu_dropdown_toggles( $item_output, $item, $depth, $args ) {
	...
	*/
	...
	// Add SVG icon.
	$dropdown_button .= $expanded ? chaser_get_svg( 'collapse' ) : chaser_get_svg( 'expand' );
	...
	$dropdown_button .= sprintf(
		...
		esc_attr( sprintf( "$expanded_state_id ? %s : %s", wp_json_encode( esc_html__( 'Collapse child menu', 'chaser' ) ), wp_json_encode( esc_html__( 'Expand child menu', 'chaser' ) ) ) ),
		esc_html( $expanded ? esc_html__( 'Collapse child menu', 'chaser' ) : esc_html__( 'Expand child menu', 'chaser' ) )
	);
...
add_filter( 'walker_nav_menu_start_el', 'chaser_amp_menu_dropdown_toggles', 10, 4 );
