 ...
 */
class Chaser_Magazine_Posts_Columns_Widget extends WP_Widget {
	...
	function __construct() {
		...
		// Setup Widget.
		parent::__construct(
			'chaser-magazine-posts-columns', // ID.
			esc_html__( 'Magazine (Columns)', 'chaser' ), // Name.
			array(
				'classname' => 'chaser-magazine-columns-widget',
				'description' => esc_html__( 'Displays your posts from two selected categories.', 'chaser' ),
	...
	private function default_settings() {
		...
		$defaults = array(
			'category_one'       => 0,
			'category_two'       => 0,
			'category_one_title' => esc_html__( 'Left Category', 'chaser' ),
			'category_two_title' => esc_html__( 'Right Category', 'chaser' ),
			'number'             => 4,
			'highlight_post'     => true,
		);
	...
	function render( $args, $settings ) {
		...
		// Get cached post ids.
		$post_ids_category_one = chaser_get_magazine_post_ids( $this->id . '-left-category', $settings['category_one'], $settings['number'] );
		$post_ids_category_two = chaser_get_magazine_post_ids( $this->id . '-right-category', $settings['category_two'], $settings['number'] );
		?>
	...
	function magazine_posts( $settings, $post_ids, $number_of_posts ) {
		...
		if ( $posts_query->have_posts() ) :
			...
			add_filter( 'excerpt_length', 'chaser_magazine_posts_excerpt_length' );
			...
			remove_filter( 'excerpt_length', 'chaser_magazine_posts_excerpt_length' );
	...
	function category_title( $args, $settings, $category_id, $category_title ) {
		...
		if ( ! empty( $widget_title ) ) :
			...
			$widget_title = chaser_magazine_widget_title( $widget_title, $category_id );
	...
	function update( $new_instance, $old_instance ) {
		...
		chaser_flush_magazine_post_ids();
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'category_one_title' ); ?>"><?php esc_html_e( 'Left Category Title:', 'chaser' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'category_one' ); ?>"><?php esc_html_e( 'Left Category:', 'chaser' ); ?></label><br/>
			<?php // Display Category One Select.
				$args = array(
					'show_option_all'    => esc_html__( 'All Categories', 'chaser' ),
					'show_count' 		 => true,
					'hide_empty'		 => false,
			...
			?>
				...
				<p>
			<label for="<?php echo $this->get_field_id( 'category_two_title' ); ?>"><?php esc_html_e( 'Right Category Title:', 'chaser' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'category_two' ); ?>"><?php esc_html_e( 'Right Category:', 'chaser' ); ?></label><br/>
			<?php // Display Category One Select.
				$args = array(
					'show_option_all'    => esc_html__( 'All Categories', 'chaser' ),
					'show_count' 		 => true,
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php esc_html_e( 'Number of posts:', 'chaser' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'highlight_post' ); ?>">
				...
				<?php esc_html_e( 'Highlight first post (big image + excerpt)', 'chaser' ); ?>
			</label>
		</p>
 ...
 * Register Widget
 */
function chaser_register_magazine_posts_columns_widget() {

	register_widget( 'Chaser_Magazine_Posts_Columns_Widget' );

}
add_action( 'widgets_init', 'chaser_register_magazine_posts_columns_widget' );
