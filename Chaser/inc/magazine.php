...
*/
function chaser_magazine_widgets() {
	...
	elseif ( is_customize_preview() ) :
		...
		chaser_customize_magazine_placeholder();

	elseif ( is_page_template( 'template-magazine.php' ) && current_user_can( 'edit_theme_options' ) ) :

		echo '<p class="empty-widget-area">';
		esc_html_e( 'Please go to Customize &#8594; Widgets and add at least one widget to the Magazine Homepage widget area.', 'chaser' );
		echo '</p>';
...
if ( ! function_exists( 'chaser_magazine_widget_title' ) ) :
	/**
	 ...
	 */
	function chaser_magazine_widget_title( $widget_title, $category_id ) {

		// Check if widget shows a specific category.
		if ( $category_id > 0 ) {

			// Set URL and Title for Category.
			$category_title = sprintf( esc_html__( 'View all posts from category %s', 'chaser' ), get_cat_name( $category_id ) );
...
if ( ! function_exists( 'chaser_magazine_entry_meta' ) ) :
	/**
	 * Displays the date and author of magazine posts
	 */
	function chaser_magazine_entry_meta() {
...
if ( ! function_exists( 'chaser_magazine_entry_date' ) ) :
	/**
	 * Displays the date of magazine posts
	 */
	function chaser_magazine_entry_date() {
		echo '<div class="entry-meta">' . chaser_meta_date() . '</div>';
	}
endif;
 ...
 * @return int
 */
function chaser_magazine_posts_excerpt_length( $length ) {
	return 12;
}
 ...
 */
function chaser_get_magazine_post_ids( $cache_id, $category, $number_of_posts ) {

	$cache_id = sanitize_key( $cache_id );
	$post_ids = get_transient( 'chaser_magazine_post_ids' );
	...
	if ( ! isset( $post_ids[ $cache_id ] ) || is_customize_preview() ) {
		...
		// Set Transient.
		set_transient( 'chaser_magazine_post_ids', $post_ids );
	}

	return apply_filters( 'chaser_magazine_post_ids', $post_ids[ $cache_id ], $cache_id );
}
 ...
 * @return void
 */
function chaser_flush_magazine_post_ids() {
	delete_transient( 'chaser_magazine_post_ids' );
}
add_action( 'save_post', 'chaser_flush_magazine_post_ids' );
add_action( 'deleted_post', 'chaser_flush_magazine_post_ids' );
add_action( 'customize_save_after', 'chaser_flush_magazine_post_ids' );
add_action( 'switch_theme', 'chaser_flush_magazine_post_ids' );
