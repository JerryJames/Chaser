 ...
 */
function chaser_slider_scripts() {

	// Get theme options from database.
	$theme_options = chaser_theme_options();

	// Register and enqueue FlexSlider JS and CSS if necessary.
	if ( ( true === $theme_options['slider_active'] && is_front_page() ) && ! chaser_is_amp() ) :
		...
		wp_enqueue_script( 'chaser-slider', get_template_directory_uri() . '/assets/js/slider.js', array( 'jquery-flexslider' ), '20170421' );

		// Register and enqueue slider CSS.
		wp_enqueue_style( 'chaser-slider', get_template_directory_uri() . '/assets/css/flexslider.css', array(), '20170421' );

	endif;

}
add_action( 'wp_enqueue_scripts', 'chaser_slider_scripts' );
 ...
 * @return int
 */
function chaser_slider_excerpt_length( $length ) {
	return 15;
}


if ( ! function_exists( 'chaser_slider_image' ) ) :
	/**
	 ...
	 */
	function chaser_slider_image( $size = 'post-thumbnail', $attr = array() ) {
...
if ( ! function_exists( 'chaser_slider_meta' ) ) :
	/**
	 * Displays the date and author on slider posts
	 */
	function chaser_slider_meta() {
 ...
 */
function chaser_slider_options() {
	...
	wp_localize_script( 'chaser-slider', 'chaser_slider_params', $params );

}
add_action( 'wp_enqueue_scripts', 'chaser_slider_options' );


/**
 * Display Post Slider
 */
function chaser_slider() {
